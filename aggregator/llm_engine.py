...
def _build_native_prompt(node_type, mode, director, context):
    ...
    # V16.1: AIGC 输出规范注入 (七要素 + 叙事编排 + 去AI味空洞词翻译)
    _arr = context.get("narrative_arrangement", "")
    _line = context.get("narrative_line", "")
    base += (
        "\n\n【V16.1 AIGC 输出规范 — 输出必须可直接投喂视频生成模型】\n"
        "1. 每镜按七要素组织: 参考绑定/主体+动作/空间关系/镜头(景别+角度+焦段+运镜+时长)/"
        "视觉风格(一个光照关键词+色调+瑕疵锚)/音频(具体声源枚举+同期声或配乐声明)/约束(一致性+负面排除)。\n"
        "2. 视觉代偿: 抽象动词翻译成 3-4 个镜头内可见元素; 情绪空洞词(温暖/孤独/震撼/治愈/史诗感)翻译成可拍到的具象现象。\n"
        "3. 一个光照关键词胜过十个形容词; 瑕疵锚(磨损/污渍/故障闪烁/胶片颗粒)≥2处, 真实感来自缺陷而非风格词堆砌。\n"
        "4. 结尾克制: 用'没有X, 只有Y'句式, 不堆特效不点题。\n"
        "5. 对白给三轴声学参数(发声方式/节奏/音调), 台词用全角标点控制停顿。\n"
        "6. 负面约束正向表达优先, 显式排除: 无字幕/无水印/无logo/角色外观跨镜一致。\n"
        "7. 叙事编排遵循当前设定, 时间线(现在/过去·闪回/未来·闪前/循环)与线索(A/B/C线/POV)显式标注。"
    )
    if _arr or _line:
        base += f"\n当前叙事编排: {_arr or '跟随叙事结构'} | 叙事线型: {_line or '单线'} — 按此重排场次时序并标注时间线/线索。"
