# -*- coding: utf-8 -*-
"""
ComfyUI-DirectorMaster V17.1.0 — 20 注册节点 (17 超级节点 + Final 别名 + 长篇分集接入 + 生态预案) + 600 导演库
====================================================================
...
20 注册节点 (Core 驱动 + forceInput):
 ...
 20. DirectorMasterEcoManager — 生态预案 (批次5: dm_pack包注册审计+参考素材登记(授权边界必填)+decision_log决策审计轨)
...
_NODE_SPECS = [
    ...
    ("aggregator.eco.node", "DirectorMasterEcoManager"),  # 批次5 生态预案
]
...
_ALL_DISPLAY_NAMES = {
    ...
    "DirectorMasterEcoManager": "🌱 生态预案 [三模式: dm_pack包注册审计(字段/版本/依赖三查) / 参考素材登记(授权边界必填+三列表解构) / 决策审计轨(append-only哈希链防篡改)]",
}
...
# =====================================================================
# V15.0-MERGED: 17 超级节点 + DirectorMasterFinal 别名 (共 18 注册)。
# 每个超级节点以下拉框聚合几十种模式, 能力全覆盖。
# V15.0 新增 3 个 AI 赋能节点 (共创/灵魂/直觉), 全部确定性可降级。
# V16.7 批次3 新增独立审查节点 (DirectorMasterReview, aggregator/review_engine.py)。
...
# 标记 V17.1.0 版本 (V17.1.0 批次5: 生态预案 dm_pack包注册审计/参考素材流授权边界/decision_log哈希链决策审计轨; V17.0.0 批次7: 长篇输入管线; V16.9.0 批次4: 记忆层; V16.8.0 批次6: 一致性资产化; V16.7.0 批次3: 质量闭环+管线韧性; 批次2: 知识资产工程+分镜契约基座; 批次1: LLM 链路加固+加载隔离)
__version__ = "17.1.0"
__description__ = ("V15.0-MERGED = V14.3-MERGED + AI赋能升级。"
                   ...
                   "新增4节点: CoCreator/Soul/Intuition/Fusion → 17超级节点+Final别名=18注册(批次3新增DirectorMasterReview独立审查)。"
                   "V16.0.1修订: 恢复46个legacy细粒度节点可选兼容层(DIRECTORMASTER_LEGACY_NODES=1 → 64节点, 0加载错误)。"
                   ...
                   "批次3(质量闭环+管线韧性, 六仓二轮思想独立重写): echo回声检测入质量门+LLM经济性模式(system字节级冻结开关); "
                   "契约渲染层(契约JSON→每镜提示词确定性渲染, 三模型侧重视图); golden回放(固定种子42结构级零漂移); "
                   "产物检查点断点续跑(CheckpointStore, hash失效自动重算); 修订影响面分析(Archive提交对比上一版附impact段); "
                   "独立审查节点DirectorMasterReview(干净上下文13项清单, 编号报告+镜号证据, 确定性轨无端点可用, LLM轨可选); "
                   "判例库NP-001~012(证据指针全真实); 卖点→镜头映射+手法去重校验; benchmark/成本基准 runner; 注册口径18节点/247创作模式); "
                   "批次6(一致性资产化, 六仓二轮思想独立重写): 角色DNA档案(8维: 眼型/脸型/发型/发色/肤色/体态/标志着装/气质锚, "
                   "禁词表36词拒抽象词不落档, 跨镜注入, prompt block 段边界截断不腰斩, merge基座维度同管线禁词+白名单复核); "
                   "项目风格锚_项目风格锚(导演风格确定性拼装→core包+第3路输出双通道, 4消费方条件注入缺键零漂移); "
                   "分镜契约v2(contract_version=2合法集{1,2}, 锚定库+每镜参考槽位/锚定/机位锚, 诊断码11→14, "
                   "槽位↔prompt【参考@N】标签双射校验非法元素逐个点名, 库大小=整数键最大值+1稀疏键不误报, v1结构零漂移); "
                   "资产派生谱系(version_store快照台账, 四态词汇表完整锚定/母版缺失/派生缺失/母版已更新待同步, "
                   "变更+入库失败诚实标待同步绝不伪造一致性, 滞后确认不重复入库); "
                   "视频路由槽位协议(Seedance2.5 reference_images按参考槽位升序注入, 槽位映射折入载荷含镜头槽位逐镜记录, "
                   "缺槽诚实跳过绝不伪造占位, 非法元素如实记录损坏元素); 4模式时长口径统一(NaN/Inf守卫诚实返回, 前镜断裂链诚实断开); "
                   "零第三方依赖纪律不变(仅stdlib, 本批新增 hashlib/re/math 均标准库)。")
