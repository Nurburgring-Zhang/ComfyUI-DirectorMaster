...
- **全下拉随机模式**：全部 71 个下拉框补全 🎲 随机（导演名/模式选择器/属性下拉/目标模型/风险档位等），选 🎲 随机真实随机解析（验证 30 次 28 distinct），非随机路径确定性不被破坏
- **目标时长秒级支持**：Cinematic/Script 目标时长 INT→FLOAT（min=0.05=3秒, step=0.05），短视频用小数（0.25=15秒 / 0.5=30秒 / 1=60秒），长片用整数，总时长恒覆盖片长
- **节点全方向输出**：美术节点"全部"模式输出 美术指导+空间一致性+空间布局 三方向；声音节点"全部"输出 声音设计+音乐配乐+声音层+沉默 四方向（不再单选互斥）
- **AIGC 视频生产适配**：自动判别生产模式（参考视频 > 首尾帧 > 多参考图 > 首帧 > 文生），分镜 JSON 含 生产模式 + 每镜 AIGC 适配提示词（文生=完整描述 / 首帧=首帧锚定+运动 / 首尾帧=首尾帧锚定+过渡 / 多参考图=参考锁定+动作 / 参考视频=运动迁移），Cinematic + VideoRouter 双节点集成
...
Cinematic 节点新增「直觉风险」下拉（ND/safe/medium/bold/chaotic）。

---

## 特性

- **导演风格数据库**：600 位导演 × 12 维风格档案 (含 V15 扩容 66 位 17 维)（镜头/光/节奏/色彩/表演/构图/声音/情绪/物件/年代/5维标签），选导演即锁定其风格，支持模糊搜索（王家卫 / Kubrick / 诺兰 均可）
